...
export class EventHelper {
    static ensureSubscriptions(config: ConfigType): RuntimeConfigType {
        ...
        // @ts-ignore
        let clone: RuntimeConfigType = { ...config };
        clone.events = {
            ...
            onAuthExpired: new EventSubscription<(authRefreshRequest: AuthRefreshRequest) => Promise<AuthCredentials | null>>(() => clone.onAuthExpired),
            ...
            onSelectedFramesLayoutChanged: new EventSubscription<(states: FrameLayoutType[]) => void>(() => clone.onSelectedFramesLayoutChanged),
            onSelectedFramesContentChanged: new EventSubscription<(state: Frame[]) => void>(() => clone.onSelectedFramesContentChanged),
            onPageSelectionChanged: new EventSubscription<() => void>(() => clone.onPageSelectionChanged),
            onSelectedLayoutPropertiesChanged: new EventSubscription<(state: LayoutPropertiesType) => void>(() => clone.onSelectedLayoutPropertiesChanged),
            onSelectedLayoutUnitChanged: new EventSubscription<(unit: MeasurementUnit) => void>(() => clone.onSelectedLayoutUnitChanged),
            onScrubberPositionChanged: new EventSubscription<(state: AnimationPlaybackType) => void>(() => clone.onScrubberPositionChanged),
            onFrameAnimationsChanged: new EventSubscription<(animationState: FrameAnimationType[]) => void>(() => clone.onFrameAnimationsChanged),
            onVariableListChanged: new EventSubscription<(variableList: Variable[]) => void>(() => clone.onVariableListChanged),
            onSelectedToolChanged: new EventSubscription<(tool: ToolType) => void>(() => clone.onSelectedToolChanged),
            onUndoStackStateChanged: new EventSubscription<(undoStackState: UndoState) => void>(() => clone.onUndoStackStateChanged),
            onSelectedLayoutFramesChanged: new EventSubscription<(frames: SelectedLayoutFrame[]) => void>(() => clone.onSelectedLayoutFramesChanged),
            onSelectedTextStyleChanged: new EventSubscription<(styles: SelectedTextStyle) => void>(() => clone.onSelectedTextStyleChanged),
            onColorsChanged: new EventSubscription<(colors: DocumentColor[]) => void>(() => clone.onColorsChanged),
            onParagraphStylesChanged: new EventSubscription<(paragraphStyles: ParagraphStyle[]) => void>(() => clone.onParagraphStylesChanged),
            onCharacterStylesChanged: new EventSubscription<(characterStyles: CharacterStyle[]) => void>(() => clone.onCharacterStylesChanged),
            onFontFamiliesChanged: new EventSubscription<(fontFamilies: DocumentFontFamily[]) => void>(() => clone.onFontFamiliesChanged),
            onSelectedLayoutIdChanged: new EventSubscription<(layoutId: string) => void>(() => clone.onSelectedLayoutIdChanged),
            onLayoutsChanged: new EventSubscription<(layouts: LayoutListItemType[]) => void>(() => clone.onLayoutsChanged),
            onConnectorEvent: new EventSubscription<(event: ConnectorEvent) => void>(() => clone.onConnectorEvent),
            onConnectorsChanged: new EventSubscription<(connectors: ConnectorInstance[]) => void>(() => clone.onConnectorsChanged),
            ...
            onShapeCornerRadiusChanged: new EventSubscription<(cornerRadius: CornerRadiusUpdateModel) => void>(() => clone.onShapeCornerRadiusChanged),
            onCropActiveFrameIdChanged: new EventSubscription<(id?: Id) => void>(() => clone.onCropActiveFrameIdChanged),
            ...
            onBarcodeValidationChanged: new EventSubscription<(validationResults: BarcodeFrameValidationResult[]) => void>(() => clone.onBarcodeValidationChanged),
        };
        return clone;
    }
