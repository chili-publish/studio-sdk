...
describe('EventSubscription', () => {
  let subscription: EventSubscription<(...args: any[]) => any>;
...
describe('EventHelper', () => {
  test('should ensure subscriptions', () => {
    ...
    const runtimeConfig = EventHelper.ensureSubscriptions({
      onActionsChanged: (state) => { counter++; },
    })
  ...
  test('old (broken) workflow should remain intact', () => {
    ...
    let incomingConfig = {
      onActionsChanged: (state: DocumentAction[]) => { counter++; },
    };
    ...
    sdk.config.onActionsChanged = (state) => { counter = 100 };
